...
pcl::PointCloud<pcl::PointXYZ>::Ptr
    cloud_after_Statics(new pcl::PointCloud<pcl::PointXYZ>);
...
void StatisticalOutlierFilter(const int &meanK, const double &stddevThre);
...
int main(int argc, char **argv) {
  ...
  private_nh.param("path_to_map", pcd_file,
                   std::string("/home/mira/Downloads/LOAM/GlobalMap.pcd"));
  ROS_INFO("*** pcd_file = %s ***\n", pcd_file.c_str());
  ...
  private_nh.param("meanK", meanK, 50);
  private_nh.param("static_stddev", static_stddev, 1.0);
  ...
  StatisticalOutlierFilter(meanK, static_stddev);
...
void RadiusOutlierFilter(const pcl::PointCloud<pcl::PointXYZ>::Ptr &pcd_cloud0,
  ...
  radiusoutlier.setRadiusSearch(radius); // 设置半径为 radius 的范围内找临近点
  radiusoutlier.setMinNeighborsInRadius(
      thre_count); // 设置查询点的邻域点集数小于 thre_count 的删除
...
void SetMapTopicMsg(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
  ...
  for (int iter = 0; iter < cloud->points.size(); iter++) {
    // check if outbound the set map size
    ...
    // -1==unknown [0,100], OccupancyThre descide by map.yaml
...
void StatisticalOutlierFilter(const int &meanK, const double &stddevThre) {
  ...
  // set neighbors to analize
  sor.setMeanK(meanK);
  sor.setStddevMulThresh(stddevThre);
  sor.filter(*cloud_after_Statics);
}
